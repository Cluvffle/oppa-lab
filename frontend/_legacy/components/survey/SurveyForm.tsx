...
const STEPS = [
  { num: 1, title: '기본 정보 📋', sub: '부담 없이 답해줘도 돼요. 닉네임으로 해도 OK!' },
  { num: 2, title: '외적 매력 점검 👔', sub: '솔직하게 체크해줄수록 정확한 진단이 가능해요' },
  { num: 3, title: '내면 & 대화 능력 🗣️', sub: '연애에서 가장 중요한 영역이에요' },
  { num: 4, title: '연애 이력 & 현재 상황 💔', sub: '판단 절대 없어요. 솔직할수록 좋아요' },
  { num: 5, title: '코칭 목표 🎯', sub: '마지막이에요! 연락처 남겨주시면 리포트 보내드려요' },
]
...
export default function SurveyForm() {
  ...
  const stepsContent = [
    ...
    <div className="survey-questions" key="step1">
      ...
      <div>
        ...
        <input
          className="q-input"
          placeholder="예) 28살, 회사원 / 32살, 프리랜서"
          value={data.age_job}
          onChange={(e) => set('age_job', e.target.value)}
        />
      </div>
      <div>
        ...
        <OptionSelect
          ...
          onChange={(v) => set('relationship_status', v as string)}
          options={[
            '모태솔로 (연애 경험 없음)',
            '연애 경험 있음 (3년 이상 전)',
            '연애 경험 있음 (최근 1~3년)',
      ...
      <div>
        ...
        <OptionSelect
          name="income"
          value={data.income}
          onChange={(v) => set('income', v as string)}
    ...
    <div className="survey-questions" key="step2">
      <div>
        ...
        <OptionSelect
          ...
          onChange={(v) => set('fashion_level', v as string)}
          options={[
            '😅 거의 신경 안 씀 (편한 것 위주)',
            '🙂 기본 수준 (깔끔한 캐주얼)',
            '😊 나름 신경 씀 (트렌드 의식)',
      ...
      <div>
        ...
        <OptionSelect
          ...
          onChange={(v) => set('grooming_level', v as string)}
          options={[
            '😅 전혀 신경 안 씀',
            '🙂 기본 세안/샴푸 정도',
            '😊 스킨케어 루틴 있음',
      ...
      <div>
        ...
        <OptionSelect
          ...
          onChange={(v) => set('fitness_level', v as string)}
          options={[
            '😅 거의 안 함',
            '🙂 가끔 (월 1~2회)',
            '😊 주 1~2회 꾸준히',
    ...
    <div className="survey-questions" key="step3">
      <div>
        ...
        <OptionSelect
          ...
          onChange={(v) => set('talk_feeling', v as string)}
          options={[
            '😰 심하게 긴장하고 말이 막힌다',
            '😅 어색하지만 어느 정도는 된다',
            '🙂 친구처럼 편하게 대화 가능',
      ...
      <div>
        ...
        <SliderInput value={data.self_esteem} onChange={(v) => set('self_esteem', v)} />
      </div>
      <div>
        ...
        <OptionSelect
          ...
          onChange={(v) => set('biggest_fear', v as string)}
          options={[
            '💔 거절당하는 것',
            '😔 상대를 실망시키는 것',
            '😬 고백 후 관계가 어색해지는 것',
    ...
    <div className="survey-questions" key="step4">
      <div>
        ...
        <OptionSelect
          ...
          onChange={(v) => set('dating_count', v as string)}
          options={['0번 (모태솔로)', '1~2번', '3~5번', '6번 이상']}
          grid
        />
      </div>
      <div>
  ...
  const handleSubmit = async () => {
    ...
    const submissionId = await submitSurvey(data)

    setLoading(false)
    router.push(`/complete/${submissionId}`)
  }
    ...
    <div className="survey-page">
      <div style={{ maxWidth: 640, margin: '0 auto' }}>
        ...
        <div className="survey-card fade-in" key={step}>
          ...
          {error && <div className="survey-error">⚠️ {error}</div>}
          ...
          <div className="survey-nav">
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? '제출 중...' : isLastStep ? '✨ 진단 신청 완료!' : '다음 →'}
            </button>
          </div>
        </div>
